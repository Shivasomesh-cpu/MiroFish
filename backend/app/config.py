...
# Path: Posiedon/.env relative to backend/app/config.py
project_root_env = os.path.join(os.path.dirname(__file__), "../../.env")

if os.path.exists(project_root_env):
    load_dotenv(project_root_env, override=True)
else:
    # Fall back to ambient environment variables if no root `.env` file exists.
...
class Config:
    ...
    # JSON settings: keep non-ASCII text readable in responses
    JSON_AS_ASCII = False
    ...
    # Supported OASIS platform actions
