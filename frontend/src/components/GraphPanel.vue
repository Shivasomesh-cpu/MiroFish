...
.graph-panel {
  ...
  background-image: radial-gradient(var(--mf-border-subtle) 1.5px, transparent 1.5px);
...
.panel-header {
  ...
  align-items: center;
  background: linear-gradient(to bottom, var(--mf-bg-secondary), transparent);
  pointer-events: none;
}
...
.slider {
  ...
  right: 0;
  bottom: 0;
  background-color: var(--mf-bg-tertiary, #1a1a2e);
  border-radius: 22px;
  transition: 0.3s;
}
...
.slider:before {
  ...
  left: 3px;
  bottom: 3px;
  background-color: var(--mf-text-muted, #6b7280);
  border-radius: 50%;
  transition: 0.3s;
}

input:checked + .slider {
  background-color: var(--mf-accent, #8b5cf6);
}
...
.loading-spinner {
  width: 40px;
  height: 40px;
  border: 3px solid var(--mf-border, rgba(139, 92, 246, 0.2));
