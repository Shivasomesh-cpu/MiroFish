...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // ç¯æ³¡å›¾æ ‡ - ä»£è¡¨æ´žå¯Ÿ
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // åœ°çƒå›¾æ ‡ - ä»£è¡¨å…¨æ™¯æœç´¢
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // ç”¨æˆ·å›¾æ ‡ - ä»£è¡¨å¯¹è¯
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // é—ªç”µå›¾æ ‡ - ä»£è¡¨å¿«é€Ÿ
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // å›¾è¡¨å›¾æ ‡ - ä»£è¡¨ç»Ÿè®¡
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // æ•°æ®åº“å›¾æ ‡ - ä»£è¡¨å®žä½“
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const entitySection = text.match(/### ã€æ ¸å¿ƒå®žä½“ã€‘\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const summaryMatch = text.match(/### é‡‡è®¿æ‘˜è¦ä¸Žæ ¸å¿ƒè§‚ç‚¹\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Try the "Question X:" style first.
      const cnPattern = /(?:^|[\r\n]+)é—®é¢˜(\d+)[ï¼š:]\s*/g
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        props.result.interviews[activeIndex.value]?.selectionReason && h('div', { class: 'selection-reason' }, [
          h('div', { class: 'reason-label' }, 'é€‰æ‹©ç†ç”±'),
...
const activeStep = computed(() => {
  ...
  return steps[0] || { noLabel: '--', title: 'ç­‰å¾…å¼€å§‹', status: 'todo', meta: '' }
})
...
const getLogLevelClass = (log) => {
  if (log.includes('ERROR') || log.includes('é”™è¯¯')) return 'error'
  if (log.includes('WARNING') || log.includes('è­¦å‘Š')) return 'warning'
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // ç¡®ä¿æ¸…é™¤ loading çŠ¶æ€
...
const extractFinalContent = (response) => {
  ...
  const chineseFinalMatch = response.match(/æœ€ç»ˆç­”æ¡ˆ[:ï¼š]\s*\n*([\s\S]*)$/i)
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--mf-border, rgba(139, 92, 246, 0.2)); padding-bottom: 8px; }
...
/* Console Logs - ä¸Ž Step3Simulation.vue ä¿æŒä¸€è‡´ */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
