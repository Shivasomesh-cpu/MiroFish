<template>
  <div class="process-page">
    <!-- é¡¶éƒ¨å¯¼èˆªæ  -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">POSIEDON</div>
      
      <!-- ä¸­é—´æ­¥éª¤æŒ‡ç¤ºå™¨ -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">å›¾è°±æž„å»º</div>
      </div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">å®žæ—¶çŸ¥è¯†å›¾è°±</span>
          </div>
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- å›¾è°±å¯è§†åŒ–ï¼ˆåªè¦æœ‰æ•°æ®å°±æ˜¾ç¤ºï¼‰ -->
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">ç”Ÿæˆå®ŒæˆåŽå°†è‡ªåŠ¨å¼€å§‹æž„å»ºå›¾è°±</p>
          </div>
          
          <!-- æž„å»ºä¸­ä½†è¿˜æ²¡æœ‰æ•°æ® -->
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">å›¾è°±æž„å»ºä¸­</p>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">æž„å»ºæµç¨‹</span>
        </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">æŽ¥å£è¯´æ˜Ž</div>
                <div class="detail-content">
                  ä¸Šä¼ æ–‡æ¡£åŽï¼ŒLLMåˆ†æžæ–‡æ¡£å†…å®¹ï¼Œè‡ªåŠ¨ç”Ÿæˆé€‚åˆèˆ†è®ºæ¨¡æ‹Ÿçš„æœ¬ä½“ç»“æž„ï¼ˆå®žä½“ç±»åž‹ + å…³ç³»ç±»åž‹ï¼‰
                </div>
              </div>
              
              <!-- æœ¬ä½“ç”Ÿæˆè¿›åº¦ -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">ç”Ÿæˆè¿›åº¦</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">ç”Ÿæˆçš„å®žä½“ç±»åž‹ ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">ç”Ÿæˆçš„å…³ç³»ç±»åž‹ ({{ projectData.ontology.relation_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">å›¾è°±æž„å»º</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">æŽ¥å£è¯´æ˜Ž</div>
                <div class="detail-content">
                  åŸºäºŽç”Ÿæˆçš„æœ¬ä½“ï¼Œå°†æ–‡æ¡£åˆ†å—åŽè°ƒç”¨ Zep API æž„å»ºçŸ¥è¯†å›¾è°±ï¼Œæå–å®žä½“å’Œå…³ç³»
                </div>
              </div>
              
              <!-- ç­‰å¾…æœ¬ä½“å®Œæˆ -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">ç­‰å¾…æœ¬ä½“ç”Ÿæˆå®Œæˆ...</div>
              </div>
              
              <!-- æž„å»ºè¿›åº¦ -->
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">æž„å»ºè¿›åº¦</div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">æž„å»ºç»“æžœ</div>
                <div class="build-result">
                  <div class="result-item">
                    <span class="result-value">{{ graphData.node_count }}</span>
                    <span class="result-label">å®žä½“èŠ‚ç‚¹</span>
                  </div>
                  <div class="result-item">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">å®žä½“ç±»åž‹</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">æž„å»ºå®Œæˆ</div>
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">é¡¹ç›®åç§°</span>
...
// å½“å‰é¡¹ç›®IDï¼ˆå¯èƒ½ä»Ž'new'å˜ä¸ºå®žé™…IDï¼‰
...
const ontologyProgress = ref(null) // æœ¬ä½“ç”Ÿæˆè¿›åº¦
const currentPhase = ref(-1) // -1: ä¸Šä¼ ä¸­, 0: æœ¬ä½“ç”Ÿæˆä¸­, 1: å›¾è°±æž„å»º, 2: å®Œæˆ
...
const statusText = computed(() => {
  if (error.value) return 'æž„å»ºå¤±è´¥'
  if (currentPhase.value >= 2) return 'æž„å»ºå®Œæˆ'
  if (currentPhase.value === 1) return 'å›¾è°±æž„å»ºä¸­'
  if (currentPhase.value === 0) return 'æœ¬ä½“ç”Ÿæˆä¸­'
  return 'åˆå§‹åŒ–ä¸­'
})
...
const goToNextStep = () => {
  // TODO: è¿›å…¥çŽ¯å¢ƒæ­å»ºæ­¥éª¤
  alert('çŽ¯å¢ƒæ­å»ºåŠŸèƒ½å¼€å‘ä¸­...')
}
...
// åˆå§‹åŒ– - å¤„ç†æ–°å»ºé¡¹ç›®æˆ–åŠ è½½å·²æœ‰é¡¹ç›®
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // æ–°å»ºé¡¹ç›®ï¼šä»Ž store èŽ·å–å¾…ä¸Šä¼ çš„æ•°æ®
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'æ²¡æœ‰å¾…ä¸Šä¼ çš„æ–‡ä»¶ï¼Œè¯·è¿”å›žé¦–é¡µé‡æ–°æ“ä½œ'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // æœ¬ä½“ç”Ÿæˆé˜¶æ®µ
    ontologyProgress.value = { message: 'æ­£åœ¨ä¸Šä¼ æ–‡ä»¶å¹¶åˆ†æžæ–‡æ¡£...' }
    ...
    if (response.success) {
      ...
      // è‡ªåŠ¨å¼€å§‹å›¾è°±æž„å»º
      await startBuildGraph()
    } else {
      error.value = response.error || 'æœ¬ä½“ç”Ÿæˆå¤±è´¥'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'é¡¹ç›®åˆå§‹åŒ–å¤±è´¥: ' + (err.message || 'æœªçŸ¥é”™è¯¯')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'æ­£åœ¨å¯åŠ¨å›¾è°±æž„å»º...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'å›¾è°±æž„å»ºä»»åŠ¡å·²å¯åŠ¨...'
      
      // ä¿å­˜ task_id ç”¨äºŽè½®è¯¢
      const taskId = response.data.task_id
      
      // å¯åŠ¨å›¾è°±æ•°æ®è½®è¯¢ï¼ˆç‹¬ç«‹äºŽä»»åŠ¡çŠ¶æ€è½®è¯¢ï¼‰
      startGraphPolling()
      
      // å¯åŠ¨ä»»åŠ¡çŠ¶æ€è½®è¯¢
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'å¯åŠ¨å›¾è°±æž„å»ºå¤±è´¥'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'å¯åŠ¨å›¾è°±æž„å»ºå¤±è´¥: ' + (err.message || 'æœªçŸ¥é”™è¯¯')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // å…ˆèŽ·å–é¡¹ç›®ä¿¡æ¯ä»¥èŽ·å– graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('âœ… å›¾è°±æž„å»ºå®Œæˆï¼Œæ­£åœ¨åŠ è½½å®Œæ•´æ•°æ®...')
        
        stopPolling()
        stopGraphPolling()
        currentPhase.value = 2
        
        // æ›´æ–°è¿›åº¦æ˜¾ç¤ºä¸ºå®ŒæˆçŠ¶æ€
        buildProgress.value = {
          progress: 100,
          message: 'æž„å»ºå®Œæˆï¼Œæ­£åœ¨åŠ è½½å›¾è°±...'
        }
        
        // é‡æ–°åŠ è½½é¡¹ç›®æ•°æ®èŽ·å– graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('ðŸ“Š åŠ è½½å®Œæ•´å›¾è°±:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'å›¾è°±æž„å»ºå¤±è´¥: ' + (task.error || 'æœªçŸ¥é”™è¯¯')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // ä¿å­˜åŽŸå§‹æ•°æ®
  }))
  
  // åˆ›å»ºèŠ‚ç‚¹IDé›†åˆç”¨äºŽè¿‡æ»¤æœ‰æ•ˆè¾¹
  ...
  // ç»˜åˆ¶è¾¹ï¼ˆåŒ…å«å¯ç‚¹å‡»çš„é€æ˜Žå®½çº¿ï¼‰
  ...
  simulation.on('tick', () => {
    // æ›´æ–°æ‰€æœ‰è¾¹çš„ä½ç½®ï¼ˆåŒ…æ‹¬å¯è§çº¿å’Œé€æ˜Žç‚¹å‡»åŒºåŸŸï¼‰
